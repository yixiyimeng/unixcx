    ...
    <view class="mpvue-picker">
        ...
        <view class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
            ...
            <picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChangeMul" v-if="mode==='multiLinkageSelector' && deepLength===2">
                <block>
                    <picker-view-column>
                        <view class="picker-item" v-for="(item,index) in pickerValueMulTwoOne" :key="index">{{item.name}}</view>
                    </picker-view-column>
                    <picker-view-column>
                        <view class="picker-item" v-for="(item,index) in pickerValueMulTwoTwo" :key="index">{{item.name}}</view>
                    </picker-view-column>
                </block>
            </picker-view>
            <!-- 三级联动 -->
        ...
        methods: {
            initPicker(valueArray) {
                ...
                if (this.mode === 'selector') {
                    ...
                    if (this.pickerValueDefault.length === 2) {
                        ...
                        for (
                            let i = 0, length = pickerValueArray[num].cities.length; i < length; i++
                        ) {
                            pickerValueMulTwoTwo.push(pickerValueArray[num].cities[i]);
                        }
                    } else {
                        for (
            ...
            pickerChangeMul(e) {
                if (this.deepLength === 2) {
                    ...
                    if (changeValue[0] !== this.pickerValue[0]) {
                        let pickerValueMulTwoTwo = [];
                        // 第一列滚动第二列数据更新
                        for (let i = 0, length = pickerValueArray[changeValue[0]].cities.length; i < length; i++) {
            ...
            _getPickerLabelAndValue(value, mode) {
                ...
                if (mode === 'selector') {
                    ...
                    pickerLable =
                        this.deepLength === 2 ?
                        `${this.pickerValueMulTwoOne[value[0]].name}-${this.pickerValueMulTwoTwo[value[1]]?this.pickerValueMulTwoTwo[value[1]].name:this.pickerValueMulTwoOne[value[0]].name}` :
