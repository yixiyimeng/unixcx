	...
	<view class="content">
		<uni-nav-bar :title="title">
			<template slot="left">
				...
				<!-- 未读消息提醒 -->
				<navigator hover-class="none" url="/pages/home/news/index" class="noReadMsg" v-if="HomeData.NewMsg > 0">
					<image src="/static/images/noReadMsg.png" />
				</navigator>
			</template>
		</uni-nav-bar>
		<!-- search -->
		...
		<div :style="{ height: scrollhight + 'px', top: offsetTop + 'px' }" class="wrap">
			<mescroll-uni :down="downOption" :up="upOption" @down="downCallback" @up="upCallback" :fixed="false" @init="mescrollInit">
				<!-- swiper -->
				<view class="swiper">
					<swiper style="height:100%" autoplay="ture" interval="3000" duration="500" indicator-dots="true"
					 indicator-active-color="#ffcc00" indicator-color="#fff" circular="true">
						<swiper-item>
				...
				<!-- 推荐新闻 -->
				<navigator class="newsBox" :url="`/pages/sjjsAbout/news/details?id=${HomeData.ImgNews[0].Id}`" hover-class="none"
				 v-if="HomeData.ImgNews && HomeData.ImgNews.length > 0">
					<view class="left_img">
						<image mode="widthFix" :src="HomeData.ImgNews[0].Img" />
					</view>
				...
				<view class="uni-swiper-msg " v-if="HomeData.News && HomeData.News.length > 0">
					<view class="uni-swiper-msg-icon">
						<image src="/static/images/horn.png" mode="widthFix"></image>
					</view>
				...
				<!-- 墓园列表 -->
				<div class="list">
					<cemetery-info :cemetery-item="item" v-for="(item, index) in cemetery_list" :key="index"></cemetery-info>
				</div>
			</mescroll-uni>
		</div>
	...
	export default {
		...
		methods: {
			...
			onConfirm(e) {
				...
				uni.setStorageSync('addressData', {
					provinceNmae: this.provinceName,
